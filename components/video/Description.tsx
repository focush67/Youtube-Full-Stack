...
const Description:React.FC<DescriptionProps> = ({video}) => {
  ...
  return (
    <div className={`bg-neutral-800 rounded-xl p-3 overflow-hidden ${expanded ? "h-fit"  : "line-clamp-2 max-h-28"}`}>
        ...
        <p onClick={() => setexpanded((prev) => !prev)} className={`cursor-pointer ${expanded ? "mt-2" : "" }`}>{
