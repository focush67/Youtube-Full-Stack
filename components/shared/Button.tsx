...
const Button:React.FC<PropsWithChildren<ButtonProps>> = ({onClick,type,className,children}) => {
  ...
  const typeClassName = useMemo(() => {
    switch(type)
    {
        ...
        case "rounded":
            return "text-stone-950 bg-zinc-300 font-medium rounded-full px-3 py-2";
  ...
  return (
    <button className={` ${typeClassName} ${className}`} onClick={onClick}>{children}</button>
  )
}
